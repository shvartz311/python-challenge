...
def generateToken(func):
    def inner():
        ...
        global refresh_token; refresh_token = InitialGetTokenJson['refresh_token']
...
def getStoryIntoCsv(listOfUris, story):
    try:
        GetContentOfFile = requests.get(url=f'{platform_base_url}/artifactory/{repo_name}{story}',
        ...
        if GetContentOfFile.status_code == 200:
            ...
            for content in ContentOfFile:
                if string_to_find in content.decode():
                    ...
                    #final_lst = lambda T: final_lst.append(f'\"{platform_base_url}{story}\" | \"{repr(GetContentOfFile.content.decode())}\"\n') if string_to_find in content.decode() else None
                else:
                    break
    except Exception:
        pass
